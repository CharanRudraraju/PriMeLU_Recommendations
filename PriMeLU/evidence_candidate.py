...
def selection(primelu, master_path, topk):
    ...
    for j in list(range(dataset_size)):
        ...
        for support_x, support_y, item_id in zip(support_xs, support_ys, item_ids):
            ...
            norm = primelu.get_weight_avg_norm(support_x, support_y, config['inner'])
            try:
    ...
    for item_id in grad_norms.keys():
        ...
        grad_norms[item_id]['final_score'] = 0.4 * grad_norms[item_id]['discriminative_value'] + 0.6 * grad_norms[item_id]['popularity_value']
